...
§pdex-226: for another member of the health plan, they **SHOULD** provide appropriate credentials to enable the exchange §
for that individual. 
...
[invoke a SMART App](https://cds-hooks.hl7.org/1.0/#link). §pdex-227: Support for this option by Health Plan systems **SHOULD** be provided. § 
§pdex-228: The SMART on FHIR app provided as a link from the returned CDS Hook **SHOULD** enable a clinician to §
...
§pdex-229: All requesters (e.g., EHRs) **SHOULD** store provenance associated with any data exchanged as part of this IG if §
...
the FHIR API is to use OAuth2.0 as the Authorization protocol. §pdex-230: The member **SHALL** authenticate using §
...
§pdex-231: While authenticated to the application or service, the member **MAY** connect to the (source) Health Plan's §
authorization server. After authenticating to the (source) Health Plan's authorization server, the Member 
§pdex-232: **SHALL** be presented with an Authorization screen that enables them to approve the sharing of information with §
...
§pdex-233: The Authorization process, in accordance with applicable privacy policy, **SHALL** provide a mechanism to §
...
§pdex-234: After successfully authorizing an application an Access Token and Optional Refresh Token **SHALL** be §
...
§pdex-236: Any subsequent Access Token issued based on the Refresh Token **SHALL** be restricted (at least) to the same §
restrictions.

§pdex-237: The requesting application **SHALL** use the access token to access the Health Plan's secure FHIR API to download §
...
§pdex-238: Clients wishing to retrieve data **SHOULD** consult the Data Provider's Server Capability Statement to §
...
§pdex-240: Payers **MAY** choose to exchange unstructured data with Patients and Providers, via their respective APIs, §
