...
the Rendering Provider. §pdex-291: Health plans **MAY** choose to construct alternative or additional Attribution §
...
The Health Plan is responsible for managing Attribution Lists. §pdex-294: Attribution Lists **SHOULD** be §
...
may use different EMRs. §pdex-295: A health plan **MAY** adopt their own methodology for managing and maintaining §
...
§pdex-303: The [$davinci-data-export operations]({{site.data.fhir.ver.atr}}/OperationDefinition-davinci-data-export.html) **SHALL** be submitted using a HTTP POST. §
...
Members to Providers. §pdex-306: The payer **SHALL** take account of members that have chosen to Opt-out of §
sharing data with providers. §pdex-307: Those opted-out members **SHALL** be omitted from the Provider §
Attribution list. §pdex-308: The Payer **MAY** choose to maintain a separate Group resource for a Provider §
...
§pdex-310: The [PDexProviderGroup](StructureDefinition-pdex-provider-group.html) profile **SHALL** be used to record the §
...
providers. §pdex-311: By recording this quantity it **MAY** help providers reconcile their attribution lists §
...
§pdex-316: - **SHALL** be issued with OAuth2.0/SMART-On-FHIR client credentials that enable access to /Group/{id}. § Where {id} is the PDexProviderGroup(s) attributed to the Organization, Facility and Provider they are representing.
§pdex-317: - **SHALL** be permitted to SEARCH /Group. § §pdex-318: The search function and the bundle contents returned **SHALL** be restricted to the {ids} that are associated with the Provider Representative's account. § 
...
in the Group. §pdex-324: If the patient parameter is provided the operation **SHALL** ignore references §
...
required. §pdex-331: It is expected that providers **MAY** first request data for members without §
limiting the request using the _since parameter. §pdex-332: Subsequent requests **MAY** then use _since §
...
§pdex-334: The _type parameter **MAY** be used to restrict the resources retrieved by the Provider. § This 
...
including an access token with scopes that cover all resources being exported. §pdex-345: A server **MAY** §
additionally restrict Bulk Data Status Request and Bulk Data Output File Requests by limiting 
them to the client that originated the export. §pdex-346: Health plans **SHALL** limit the data returned to §
...
resources and perform Bulk export operations. §pdex-348: Access Tokens **SHALL** be required to access the Group §
resources and and the Bulk export operation. §pdex-349: Access and Refresh Tokens **SHOULD** be issued to support §
