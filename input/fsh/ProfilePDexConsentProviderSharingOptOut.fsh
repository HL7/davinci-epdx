...
* scope MS
* scope ^short = "Information-disclosure scope (fixed to patient-privacy)"
* scope ^definition = "Fixed to `http://terminology.hl7.org/CodeSystem/consentscope#patient-privacy` (\"Agreement to collect, access, use or disclose (share) information\"). `Consent.scope` is structurally a \"what kind of consent is this?\" element bound by FHIR R4 to the [ConsentScope](http://terminology.hl7.org/CodeSystem/consentscope) code system (`treatment` / `patient-privacy` / `research` / `adr`); it is **not** a Purpose-of-Use field. The [v3 PurposeOfUse ValueSet](http://terminology.hl7.org/ValueSet/v3-PurposeOfUse) used by TEFCA applies to `Consent.provision.purpose`, not to `Consent.scope`. This profile does not redefine `provision.purpose`, so implementers retain the base R4 Consent profile's `extensible` binding to v3 PurposeOfUse on that element and may use TEFCA-style purpose codes there as appropriate."
