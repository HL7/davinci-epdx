# Change History - Da Vinci Payer Data Exchange v2.2.0
...
| JIRA Ticket | Change | |———————————————————|—————————————————————————————————–| | [FHIR-56075](https://jira.hl7.org/browse/FHIR-56075) | Resolve discrepancy between $provider-member-match OperationDefinition and Parameters IN profile: rename part parameter from `Consent` to `TreatmentAttestation` and container from `MemberBundle` to `MembersToMatch`; add links to IN/OUT Parameters profiles from operation definition page | | [FHIR-55987](https://jira.hl7.org/browse/FHIR-55987) | Remove requirements to record export operation details (lastTransmitted, lastResources, lastFilters) as extensions on Patient/Group resources; replace with SHOULD guidance to use internal audit trails; remove LastTransmission, LastTypes and LastFilters extensions | | [FHIR-55981](https://jira.hl7.org/browse/FHIR-55981) | Add STU note to $bulk-member-match async section acknowledging that the R4 async pattern (ndjson manifest) is an imperfect fit for this operation; note that FHIR R5/R6 async-bundle is a more natural pattern; state that when PDex moves to FHIR R6 the async interaction pattern will be reconsidered in light of the R6 async specification | | [FHIR-55928](https://jira.hl7.org/browse/FHIR-55928) | Resolve conflicting conformance verbs for single-member Payer-to-Payer Exchange: promote §pdex-176 from MAY to SHALL (consistent with §pdex-182) making single-member exchange a required capability alongside bulk exchange | | [FHIR-55927](https://jira.hl7.org/browse/FHIR-55927) | Add optional `MatchCoverage` extension (`base-ext-match-coverage`, 0..1) on `Group.member.entity` to allow the submitted Coverage to be returned alongside the submitted Patient in bulk-member-match response Groups; add `matchedCoverage`/`nonMatchedCoverage` extension slices (0..1 MS) to `PDexMemberMatchGroup` and `PDexMemberNoMatchGroup`; add §pdex-139d requiring Coverage to be included when the same patient is submitted with multiple coverages; update example to illustrate the pattern | | [FHIR-55640](https://jira.hl7.org/browse/FHIR-55640) | TC: Remove duplicate/conflicting provider member match examples — delete `Provider-Bulk-Member-Match-Request-Example` (used obsolete `MemberBundle`/`Consent` names, 1 member) from ExampleProviderTreatmentAttestation.fsh and delete ExampleProviderMultiMemberMatchResponse.fsh; canonical examples are now `provider-member-match-request-001` (2 members, `MembersToMatch`/`TreatmentAttestation`) and `provider-member-match-response-001`, both aligned with the ProviderMemberMatch OperationDefinition | | [FHIR-55620](https://jira.hl7.org/browse/FHIR-55620) | TC: Remove `CoverageToLink` from the Provider-Member-Match request example and delete the associated `CoverageLinkExample1` instance — providers are not payers and do not issue new coverage, so this parameter has no applicable use in the provider context | | [FHIR-53815](https://jira.hl7.org/browse/FHIR-53815) | Add search parameter and extension to enable easier retrieval of Provider Access Opt-Out | | [FHIR-53793](https://jira.hl7.org/browse/FHIR-53793) | Create a listing of all Conformance Requirements in the Narrative of the IG | | [FHIR-53655](https://jira.hl7.org/browse/FHIR-53655) | Expand `Consent.performer` on PDex Provider Access Consent Profile to allow `RelatedPerson` in addition to `Patient`, permitting legally recognized personal representatives (parent, guardian, healthcare proxy) to exercise opt-out on behalf of the patient per CMS-0057-F and HIPAA; add `$USCoreRelatedPerson` alias | | [FHIR-53616](https://jira.hl7.org/browse/FHIR-53616) | Add async request pattern detail to Provider Bulk Access API section: after §pdex-304 add §pdex-304a/304b describing the three-step async pattern (HTTP 202 kick-off, Content-Location polling, NDJSON retrieval); clarify the payer does not push data to the provider | | [FHIR-53311](https://jira.hl7.org/browse/FHIR-53311) | Originating ticket for Provider Access API v2: switch from static Group-based attribution lists to a Bulk Member Match first step; provider submits treatment relationship attestation (no member consent required); payer checks in-network status, member opt-out, and treatment relationship; matched members Group used with $davinci-data-export; implemented as Provider Access API v2 (provider-access-api-v2.md, OperationDefinition-ProviderMemberMatch, ProviderMultiMemberMatchRequestParameters profile) | | [FHIR-53256](https://jira.hl7.org/browse/FHIR-53256) | Clarify async delivery of $bulk-member-match response: update narrative to state that Group resources are serialized to ndjson and delivered via the async completion manifest (not returned in a synchronous response body); clarify the "Out:" profile reference; note that the OperationDefinition intentionally omits inline out parameters for the same reason as $davinci-data-export | | [FHIR-52871](https://jira.hl7.org/browse/FHIR-52871) | Require cross-reference extension in $bulk-member-match response: change `matchedMember`/`nonMatchedMember` extension cardinality from 0..1 to 1..1 on PDexMemberMatchGroup and PDexMemberNoMatchGroup profiles; add normative narrative (§pdex-139b/139c) requiring the contained Patient to be the exact submitted resource preserving original id and identifiers; fix examples to label contained Patient identifiers as "New Payer" (the requester) rather than "Old Payer" | | [FHIR-52066](https://jira.hl7.org/browse/FHIR-52066) | Clarify data mapping requirements: promote §pdex-69 clinical information mapping from SHOULD to SHALL; add §pdex-69a (claims/encounters SHALL use CARIN BB non-financial profiles) and §pdex-69b (prior authorizations SHALL use PDex Prior Authorization profile); restrict §pdex-71 fallback to clinical data only, removing erroneous reference to claims | | [FHIR-52056](https://jira.hl7.org/browse/FHIR-52056) | Fix inconsistency in Overview §pdex-117: change SHALL to SHOULD for accepting and retaining provenance records, aligning with §6.1 Handling Data Provenance and §6.5.29.1.6 (outcome of FHIR-47054) | | [FHIR-53087](https://jira.hl7.org/browse/FHIR-53087) | TC: Fix PDex Provider Group example — `managingEntity` corrected to reference the Payer organization (not a Provider organization); add comments clarifying `managingEntity` = Payer that created the Group and `characteristic.valueReference` = Provider being attributed with members | | [FHIR-51377](https://jira.hl7.org/browse/FHIR-51377) | Add `PDexConsentApiPurposeCS` CodeSystem and `PDexConsentApiPurposeVS` ValueSet with codes `provider-access`, `payer-to-payer`, `patient-access`; add `category[apiPurpose]` slice (1..1 MS) fixed to `provider-access` on PDex Provider Access Consent profile enabling standard category search to discriminate Consent records by API; add narrative to consent.md explaining disambiguation pattern and rationale for separate P2P vs Provider Access profiles | | [FHIR-51845](https://jira.hl7.org/browse/FHIR-51845) | Add custom `characteristic-value-reference` SearchParameter for Group to enable searching by `characteristic.valueReference` (no standard FHIR base search parameter covers this field); enable in Payer Access and Provider Access CapabilityStatements | | [FHIR-51571](https://jira.hl7.org/browse/FHIR-51571) | Resolved by FHIR-55987 (removal of lastTransmitted/lastResources/lastFilters extensions) which eliminates the server-side delta tracking ambiguity; clarify that when using provider-delta the client SHALL supply `_since` and is responsible for tracking previous request dates via an internal audit trail | | [FHIR-50458](https://jira.hl7.org/browse/FHIR-50458) | TC: Replace invalid `identifier.type` code `NPI` with `PRN` (Provider Number) from v2-0203 in Organization and Practitioner examples to conform to the extensible binding on `identifier.type` | | [FHIR-50227](https://jira.hl7.org/browse/FHIR-50227) | Reconcile PDex single $member-match with HRex response profile: clarify that `MemberIdentifier` (business identifier/UMB) is required per HRex and `MemberId` (Patient FHIR ID) SHOULD also be returned; fix terminology distinguishing business identifiers from Patient FHIR IDs; clarify authorization flow for both cases | | [FHIR-50184x](https://jira.hl7.org/browse/FHIR-50184) | Details related to notifications for bulk response missing for Provider access API and Payer to Payer API | | [FHIR-50183x](https://jira.hl7.org/browse/FHIR-50183) | Provider access details in the payer-payer section of the IG at 6.4.4 | | [FHIR-49943](https://jira.hl7.org/browse/FHIR-49943) | Clarify async usage of $multi-member-match: add two-step async flow overview paragraph; split ambiguous combined §pdex-136/137 statement into separate conformance requirements for $multi-member-match (async with Prefer: respond-async) and $davinci-data-export (async bulk export); update BulkMemberMatch OperationDefinition description and replace sync inline out parameters with async manifest pattern |
